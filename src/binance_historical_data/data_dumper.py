...
class BinanceDataDumper:
    ...
    def __init__(
        ...
        self.dict_new_points_saved_by_ticker: defaultdict[str, typing.Any] = (
            defaultdict(dict)
        )
    ...
    def get_list_all_trading_pairs(self) -> list[str]:
        ...
        if self._asset_class == "um":
            response = urllib.request.urlopen(
                f"https://fapi.binance.us/fapi/v1/exchangeInfo"
            ).read()
        elif self._asset_class == "cm":
            response = urllib.request.urlopen(
                f"https://dapi.binance.us/dapi/v1/exchangeInfo"
            ).read()
        else:
            # https://api.binance.us/api/v3/exchangeInfo
            response = urllib.request.urlopen(
                f"https://api.binance.us/api/v3/exchangeInfo"
            ).read()
        return list(
    ...
    def get_all_dates_with_data_for_ticker(
        ...
        list_dates_with_data: list[datetime.date] = []
        path_folder_suffix = self._get_path_suffix_to_dir_with_data(
            timeperiod_per_file, ticker
        )
    ...
    def delete_outdated_daily_results(self) -> None:
        ...
        dict_files_deleted_by_ticker: defaultdict[str, typing.Any] = defaultdict(int)
    ...
    def _download_data_for_1_ticker(
        ...
        with WorkerPool(n_jobs=processes) as pool:
            list_saved_dates = list(
                tqdm(
                    pool.imap_unordered(  # type: ignore
                        self._download_data_for_1_ticker_1_date, list_args
                    ),
                    leave=False,
                    total=len(list_args),
    ...
    def _print_short_dump_statististics(self) -> None:
        ...
        list_months_saved: list[int] = []
        list_days_saved: list[int] = []
        for ticker in self.dict_new_points_saved_by_ticker:
            dict_stats = self.dict_new_points_saved_by_ticker[ticker]
